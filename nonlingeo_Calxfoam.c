...
/* Adapter: Add header */
#include "adapter/CalxfoamInterface.h"

#define max(a,b) ((a) >= (b) ? (a) : (b))

/* Adapter: Rename the function */
void nonlingeo_Calxfoam(double **cop, ITG *nk, ITG **konp, ITG **ipkonp, char **lakonp,
	     ITG *ne, 
         ...
         char * CalxfoamParticipantName, char * dataFilename){
  ...
  //! The simulation Data for Calxfoam

  struct CalcSimulationData calcSimulationData = {
      /* 
	  ...
	  */
	  .ialset = ialset,
	  /*
	  ! ielmat(j,i): material number of layer j, for element i
	  */
      .ielmat = ielmat,
	  /*
	  ! istartset(i): pointer into ialset containing the first set member, for set i
	  */
      .istartset = istartset,
	  /*
	  ...
	  */
      .kon = kon,
	  /*
	  ! ipkon(i): (location in kon of the first node in the element connectivity list of element i)-1, for element i
	  */
      .ipkon = ipkon,
	  //! lakon(i): element label( C3D4/C3D6/ C3D8I B...), for element i
      .lakon = &lakon,
	  //! co(i,j): coordinate i of node j
      .co = co,
	  //! set(i): name of the set; this is the user defined name, for set i
      .set = set,
	   //! nset: number of sets (including surfaces)
      .nset = *nset,
	  //! ikboun(i): ordered array of the DOFs corresponding to the SPC's (DOF=8*(nodeboun(i)-1)+ndirboun(i)), for single point constrants i
      .ikboun = ikboun,
      //! ikforc(i): ordered array of the DOFs corresponding to the point loads (DOF=8*(nodeboun(i)-1)+ndirboun(i)), for point load i
      .ikforc = ikforc,
	  //! ilboun(i): original SPC number for ikboun(i), for single point constraints
      .ilboun = ilboun,
	  //! ilforc(i): original SPC number for ikforc(i), for point load i
      .ilforc = ilforc,
	  //! nboun: # SPC's
      .nboun = *nboun,
	  //! nforc: # of point loads
      .nforc = *nforc,
	  //! nelemload(1,i): element to which distributed load is applied; for distributed load i
	  //! nelemload(2,i): node for the environment temperature (only for heat transfer analyses); sector number, 
	  //! if load is real; sector number + # sectors if load is imaginary (only for modal dynamics and steady state dynamics analyses with cyclic symmetry)
      .nelemload = nelemload,
	  //! nload: # of facial distributed loads
      .nload = *nload,
	  //! sideload(i): load label; indicated element side to which load is applied, for distributed load i
      .sideload = sideload,
      //! mt : mt=mi[1]+1
      .mt = mt,
	  ...
	  /*
	    ! nmethod: 
		1 : static (linear or nonlinear);
		2 : static (linear or nonlinear);  
		3 : buckling (linear);
	  ...
	  */
	  ...
	  /*
	  ! xload(1,i)
	  magnitude of load at end of a step or, for heat transfer analyses, the convection (*FILM) or the radiation coefficient (*RADIATE)
	  ! xload(2,i)
	  the environment temperature (only for heat transfer analyses
	  */
      .xload = xload,
	  //! xforc(i): magnitude of force at end of a step, for point load i
      .xforc = xforc,
	  //! xboun(i): magnitude of constraint at end of a step, for single point constraints
      .xboun = xboun,

      .ntmat_ = ntmat_,
	  //! vold(0,j): temperature of node j at the start of an iteration (for ithermal 
	  ...
	  //! add the increment number for count
	  .iinc = &iinc,
  ...
  NNEW(f,double,neq[1]);
  //actual internal forces in DOF i due to :
  ...
  //external mechanical forces in DOF i (due to point loads and distributed loads,
  //including centrifugal and gravity loads, but excluding temperature loading and displacement loading)
  NNEW(b,double,neq[1]);
  //right hand side of the equation system : difference between fext and f in nonlinear calcultions; for linear calculations, b=finc.
  NNEW(vini,double,mt**nk);
  //temperature of node j at the start of an increment
  //displacement of node j in direction i at the start of an increment
  ...
  NNEW(xbounact,double,*nboun); //magnitude of constraint at the end of the present increment
...
//    MPADD end
  ...
  Calxfoam_Setup( dataFilename, CalxfoamParticipantName, &calcSimulationData );
  checkCalxfoamInterface();
  //getchar();
  ...
  while(1.-theta>1.e-6){
      ...
      Calxfoam_AdjustSolverTimestep( &calcSimulationData );
	  ...
	  //!read force data for this increment

	  Calxfoam_ReadCouplingData( &calcSimulationData );
...
/*		  if(nasym!=0){
	 ...
	 nzs,&nasym,&idamping,veold,adc,auc,cvini,cv);
	  ...
	  theta=theta+dtheta;   //!increase time here
	  if(dtheta>=1.-theta){
	      if(dtheta>1.-theta){
    ...
    results(co,nk,kon,ipkon,lakon,ne,v,stn,inum,stx,
		...
		calcSimulationData.fn = fn;
        memcpy(&vold[0],&v[0],sizeof(double)*mt**nk);

        Calxfoam_WriteCouplingData( &calcSimulationData );
        /* Adapter: Advance the coupling */
        //Precice_Advance( &calcSimulationData );
...
//    for(k=0;k<ntg;k++)if(inum[itg[k]-1]>0){inum[itg[k]-1]*=-1;}
  ...
  *islavsurfp=islavsurf;*pslavsurfp=pslavsurf;*clearinip=clearini;
  ...
  Calxfoam_FreeData( &calcSimulationData );
