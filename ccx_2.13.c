...
/*
 *Additional variables for the coupling with Calxfoam
 *Calxfoam is used only if a participant name is provided as a command line argument
 */

char CalxfoamParticipantName[256] = "", dataFilename[256] = "data.py";
...
if(argc==1){printf("Usage: CalculiX.exe -i jobname\n");FORTRAN(stop,());}
  ...
  for(i=1;i<argc;i++){
    ...
    // Get CalxFOAM participantName
    if(strcmp1(argv[i],"-Calxfoam-participant")==0) {
        strcpy(CalxfoamParticipantName,argv[i+1]);
        CalxfoamUsed = 1;
    }
    // Overwrite YAML config file name in case a specific file name is given on the command line
    if(strcmp1(argv[i],"-Calxfoam-config")==0) {
...
//  nprint=nprint_;
  ...
  if((istep==0)||(irstrt<0)) {
    ...
    norien=norien_;
    //!number of transform definitions
    ntrans=ntrans_;
    //!number of tie constrains
    ntie=ntie_;
//    nobject=nobject_;
    ...
    NNEW(co,double,3*nk_); //coordinates
    NNEW(kon,ITG,nkon_);   //field containing the connectivity lists of the elements in successive order
    NNEW(ipkon,ITG,ne_);   //(location in kon of the first node in the element connectivity list of element i)-1
    NNEW(lakon,char,8*ne_);// element label
    ...
    if((ne1d!=0)||(ne2d!=0)){
	NNEW(iponor,ITG,2*nkon_);  //two pointers for entry i of kon. 
                               //The first pointer points to the location in xnor preceding the normals of entry i, 
                               //the second points to the location in knor of the newly generated dependent nodes of entry i.
    ...
    NNEW(xforc,double,nforc_); //magnitude of force at end of a step
    NNEW(ikforc,ITG,nforc_); //ordered array of the DOFs corresponding to the point loads (DOF=8*(nodeboun(i)-1)+ndirboun(i))
                             //nodeboun(i)	:SPC node; ndirboun: SPC direction
    NNEW(ilforc,ITG,nforc_); //original SPC number for ikforc(i)
    ...
    NNEW(sideload,char,20*nload_); //load label; indicated element side to which load is applied
    NNEW(xload,double,2*nload_); //magnitude of load at end of a step 

    /* distributed volumetric loads */

    NNEW(cbody,char,81*nbody_); //element number or element set to which load i applies
    NNEW(idefbody,ITG,nbody_);
    NNEW(ibody,ITG,3*nbody_); //ibody(1,i): code identifying the kind of body load; 2:amplitude number for load i;3: load case number for load i
    ...
    NNEW(islavsurf,ITG,2*ifacecount+2);
      ...
      if((nmethod!=4)&&(nmethod!=5)&&(nmethod!=8)&&(nmethod!=9)&& 
       ((abs(nmethod)!=1)||(iperturb[0]<2))){
        NNEW(veold,double,mt*nk_);//velocity of node j in direction i at the start of an iteration
    }
    else{
...
//  if(icascade==0) {
  ...
  /* nmethod=12: sensitivity analysis  */
    ...
    if(CalxfoamUsed){
        ...
        if(isDynamic && !isThermalAnalysis) {
            
            printf("Starting FSI analysis via CalxFOAM");
            ...
            if(iperturb[1]==0) {
                ...
                nonlingeo_Calxfoam(&co,&nk,&kon,&ipkon,&lakon,&ne,nodeboun,ndirboun,xboun,&nboun,
                                  ...
                                  xmodal,&iaxial,&inext,&nprop,&network,orname,CalxfoamParticipantName,dataFilename);
                ...
                printf("ERROR: This simulation type is not available with CalxFOAM");
                exit(0);
            }
        }
        
        else {
            printf("ERROR: Only FSI is available with CalxFOAM");
            exit(0);
        }
    }
    else if(preciceUsed) {
